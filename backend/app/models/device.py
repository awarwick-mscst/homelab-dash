...
class Device(Base):
    ...
    proxmox_server_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
    proxmox_node: Mapped[str | None] = mapped_column(String(100), nullable=True)
    proxmox_type: Mapped[str | None] = mapped_column(String(10), nullable=True)
    is_online: Mapped[bool] = mapped_column(default=False)
    is_pinned: Mapped[bool] = mapped_column(default=False)
    pinned_port: Mapped[int | None] = mapped_column(Integer, nullable=True)
    is_monitored: Mapped[bool] = mapped_column(default=False)
    monitor_url: Mapped[str | None] = mapped_column(String(500), nullable=True)
    monitor_status: Mapped[str | None] = mapped_column(String(20), nullable=True)
